if __name__ == "__main__":
    ...
    config = {"generations": 5, "population_size": 20, "verbose": 2, "n_jobs": 1, "cv": 5, "random_state": 42}
    print("[INFO] Configuração utilizada no TPOT: ", config)

    try:
        model = TPOTClassifier(max_time_mins=60, **config)
        inicio = time.time()
        ...
        print(f"[ERRO] Erro durante o fit do TPOTClassifier: {e}")

    # 4. Métricas de Validação Cruzada
    print("\n[INFO] Calculando métricas detalhadas...")
    pipeline_final = model.fitted_pipeline_
    ...
    resultado_txt = os.path.join(f"resultado_TPOT_Pipeline.txt")
    ...
    plt.tight_layout()
    plt.savefig("confusao_tpot.png")
    plt.show()

    train_sizes, train_scores, val_scores = learning_curve(
    model.fitted_pipeline_, X_train, y_train,
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"curva_aprendizado_tpot.png")
    plt.show()
    ...
    df_learning.to_csv(f"curva_aprendizado_tpot.csv", index=False)

    shap_config = {}
    os.makedirs("results/shap", exist_ok=True)
    X_subset = pd.DataFrame(X_test[:100], columns=X_test.columns)
    explainer = shap.Explainer(model.fitted_pipeline_.predict, X_train)
    shap_values = explainer(X_subset)
    
    plt.figure()
    shap.summary_plot(shap_values, X_subset, max_display=20, show=False)
    plt.tight_layout()
    plt.savefig("results/shap/shap_tpot_dotplot.png")
    plt.close()
    print(f"[✓] Gráfico SHAP salvo em: results/shap/shap_tpot.png")
    ...
    output_path = f"results/lime/lime_tpot_2.html"
    exp.save_to_file(output_path)
    print(f"[✓] Explicação LIME salva em: {output_path}")

    perm = PermutationImportance(model.fitted_pipeline_, random_state=42).fit(X_test, y_test)

    with open("results/eli5/eli5_tpot.html", "w", encoding="utf-8") as f:
        f.write(eli5.format_as_html(eli5.explain_weights(perm, feature_names=X_test.columns.tolist())))

    print("[✓] Explicação ELI5 salva em: results/eli5/eli5_tpot.html")